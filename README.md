...
初学者が Hermes と対話（grill＝取り調べ）して要件を固め、引継ぎ書をローカルで作るためのキット。

対話は **Hermes を CLI/TUI で直接使う**（スキル `/grill` 等はこの経路で発火する）。
...
- 同梱スキル: **`/grill`**（要件を1問ずつ聞き出し → 決定を逐次記録 → 引継ぎ書を合成）、
  **`/torishirabe`**（同ロジックの刑事風）。
- 編集は即反映（コンテナ再ビルド不要）。生成物は `./hermes-config/grill/<プロジェクト名>/` に出る。
